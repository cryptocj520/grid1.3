...
class VolumeMakerServiceImpl(IVolumeMakerService):
    ...
    async def _execute_market_cycle(self) -> None:
        ...
        try:
            ...
            # 多单（BUY）：开仓用卖1价格买入
            # 空单（SELL）：开仓用买1价格卖出
            open_price = ask_price if open_side == OrderSide.BUY else bid_price
    ...
    async def _wait_for_price_change(self, initial_bid: Decimal, initial_ask: Decimal,
        ...
        Args:
            ...
            open_price: 开仓价格（用于价格差值检测）
            open_side: 开仓方向（用于确定平仓价格）
            timeout: 超时时间（秒）
            ...
            while (datetime.now() - start_time).total_seconds() < timeout:
                ...
                if current_bid != last_bid or current_ask != last_ask:
                    ...
                    if price_change_count >= required_count:
                        # 🔥 检查价格差值（如果启用）
                        if (self.config.order_mode == "market" and
                            self.config.market_min_price_change > 0 and
                            open_price is not None and
                                open_side is not None):

                            # 确定平仓价格：开多用买1平，开空用卖1平
                            close_price = current_bid if open_side == OrderSide.BUY else current_ask

                            # 计算价格差值的绝对值
                            price_diff = abs(float(close_price - open_price))

                            if price_diff < self.config.market_min_price_change:
                                # 差值不够，继续等待
                                self.logger.info(
                                    f"📊 价格变化次数已达标但差值不足 - "
                                    f"变化{price_change_count}次 >= {required_count}次，"
                                    f"开仓价: {open_price}, "
                                    f"平仓价: {close_price}, "
                                    f"差值: {price_diff:.4f} < 要求{self.config.market_min_price_change} → 继续等待")
                                # 不 return，继续循环
                                await asyncio.sleep(check_interval)
                                continue
                            else:
                                ...
                                elapsed = (datetime.now() -
                                           start_time).total_seconds()
                                self.logger.info(
                                    f"✅ 价格变化达到要求次数且差值满足({self.config.orderbook_method.upper()}) - "
                                    f"变化{price_change_count}次 >= {required_count}次，"
                                    f"开仓价: {open_price}, "
                                    f"平仓价: {close_price}, "
                                    f"差值: {price_diff:.4f} >= {self.config.market_min_price_change} "
        ...
        while not self._should_stop:
            try:
                ...
                if self.config.fill_price_method == "websocket":
                    ...
                    # 检查心跳超时（60秒无消息）
                    elif self._ws_order_last_message_time:
                        elapsed = (
                            datetime.now() - self._ws_order_last_message_time).total_seconds()
                        if elapsed > 60:
                            self.logger.warning(
                                f"⚠️ WebSocket 订单连接超时（{elapsed:.1f}秒无消息），标记为不健康")
                            self._ws_order_healthy = False
                ...
                if self.config.orderbook_method == "websocket":
                    ...
                    # 检查心跳超时（60秒无消息）
                    elif self._ws_orderbook_last_message_time:
                        elapsed = (
                            datetime.now() - self._ws_orderbook_last_message_time).total_seconds()
                        if elapsed > 60:
                            self.logger.warning(
                                f"⚠️ WebSocket 订单簿连接超时（{elapsed:.1f}秒无消息），标记为不健康")
                            self._ws_orderbook_healthy = False
